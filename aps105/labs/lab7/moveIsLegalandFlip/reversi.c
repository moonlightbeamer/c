...
int moveIsLegalandFlip(char board[][26], int n, char player, int row, int col, bool flip);

void flipTiles(char board[][26], int n, char player, int row, int col, int deltaRow, int deltaCol);
...
int main(void) {
  ...
  bool flip = false; //don't want to flip now
  if (moveIsLegalandFlip(board, n, player, row, col, flip)) {
    printf("Valid move.\n");
    flip = true;
    moveIsLegalandFlip(board, n, player, row, col, flip);
  } else {
    printf("Invalid move.\n");
  }
...
void printAvailableMoves(char board[][26], int n, char player) {
  ...
  for (int row = 0; row < n; row++) {
    for (int col = 0; col < n; col++) {
      // if move is legal
      bool flip = false; //don't want to flip
      if (moveIsLegalandFlip(board, n, player, row, col, flip)) {
...
int moveIsLegalandFlip(char board[][26], int n, char player, int row, int col, bool flip) {
  ...
  if (checkLegalInDirection(board, n, row, col, player, deltaRow, deltaCol)) {
    ...
    legal = true;
    if (flip){
      flipTiles(board, n, player, row, col, deltaRow, deltaCol);
    }
  }
  ...
  return legal;
  //returns an integer converted from boolean
  //if false, will return 0 (flip 0 tiles)
  //if true, will return 1, 2, 3, etc. (# of tiles to flip)
  //all if statements using moveIsLegalandFlip will still work
  //takes in 0 as false, and 1, 2, 3, etc. as true
}
