...
int main(void) {
  ...
  while (!gameOver) {
    if (turn == computer) {
      ...
      int result = makeMove(board, n, computer, &row, &col);
      ...
      if (result) {
        ...
        flipTiles(board, n, computer, row, col);
        // print board
        printBoard(board, n);
      }
...
int makeMove(const char board[][26], int n, char turn, int *row, int *col) {
  int maxFlippableTiles = -1;
  bool foundMove = false;
  char scratchBoard[26][26];

  // loop through all positions on the board, i = row, j = col
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      // check if the move is legal
      if (moveIsLegal(board, n, turn, i, j)) {
        // creating a scratch pad board to simulate the move
        copyBoard(board, scratchBoard, n);
        flipTiles(scratchBoard, n, turn, i, j);

        // evaluate the potential future moves after this move
        int userMoves = 0;
        int computerMoves = 0;
        ...
        for (int r = 0; r < n; r++) {
          for (int c = 0; c < n; c++) {
            if (moveIsLegal(scratchBoard, n, opponentColour(turn), r, c)) {
              userMoves++;
            }
        ...
        int score = flippableScore(board, n, turn, i, j);

        // prioritizing corner moves
        ...
        // adjust score based on potential future moves
        score += (computerMoves - userMoves) * 2;

        // update the maximum score and the selected move
        if (score > maxFlippableTiles) {
          maxFlippableTiles = score;
          *row = i;
          *col = j;
          foundMove = true;
        }
